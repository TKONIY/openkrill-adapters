"""Claude Code adapter — delegates to CLI or Session CLI adapter.

Branded adapter for Anthropic's Claude Code coding agent.
Delegates all functionality to CliAdapter (cli mode) or SessionCliAdapter
(session mode) based on the ``mode`` config field.

Config keys:
    mode: str — "cli" or "session" (default: "session")
    command: str — Override CLI command (default: "claude")
...
class ClaudeCodeAdapter(BaseAdapter):
    """Branded adapter for Claude Code (Anthropic's coding agent)."""

    COMMAND = "claude"
    CLI_ARGS = "-p"  # print mode for CLI
    SESSION_ARGS = ""  # session mode args
    TYPE_NAME = "claude-code"

    def __init__(self, config: dict) -> None:
        super().__init__(config)
        self._mode = config.get("mode", "session")  # "cli" or "session"
